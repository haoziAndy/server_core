...
void CCRakNetSlidingWindow::Init(CCTimeType curTime, uint32_t maxDatagramPayload)
{
	...
	lowestRtt=lastRtt=estimatedRTT=deviationRtt=UNSET_TIME_US;
...
CCTimeType CCRakNetSlidingWindow::GetRTOForRetransmission(unsigned char timesSent) const
{
 ...
 #if CC_TIME_TYPE_BYTES==4
     // fps game, max maxThreshold set to 150
 	//const CCTimeType maxThreshold=2000;
     const CCTimeType maxThreshold=150;
 	//const CCTimeType minThreshold=100;
 	//const CCTimeType additionalVariance=30;
     const CCTimeType additionalVariance=2;
 #else
 	//const CCTimeType maxThreshold=2000000;
     const CCTimeType maxThreshold=150000;
 	//const CCTimeType minThreshold=100000;
 	//const CCTimeType additionalVariance=30000;
     const CCTimeType additionalVariance=2000;
 #endif

	if (estimatedRTT==UNSET_TIME_US)
        return maxThreshold;

    CCTimeType ccmaxThreshold = lowestRtt * 3;
    if (ccmaxThreshold > maxThreshold)
        ccmaxThreshold = maxThreshold;
 	//double u=1.0f;
// 	double u=2.0f;
//  	double q=4.0f;
    double u=1.0f;
	...
	if (threshhold > ccmaxThreshold)
		return ccmaxThreshold;
	return threshhold;
}
// ----------------------------------------------------------------------------------------------------------------------------
